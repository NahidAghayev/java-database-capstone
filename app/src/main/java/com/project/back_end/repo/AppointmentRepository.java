...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    @Query("SELECT a FROM Appointment a LEFT JOIN FETCH a.patient LEFT JOIN FETCH a.doctor WHERE a.doctor.id = :doctorId AND LOWER(a.patient.name) LIKE CONCAT('%', LOWER(:patientName), '%') AND a.appointmentTime BETWEEN :start AND :end")
